...
class GDAMamba(nn.Module):
    def __init__(self, input_dim, hidden_dim=32, num_classes=2, patch_size=1, group_num=4, blocks=3):
        ...
        self.cls_head = nn.Linear(in_features=hidden_dim[-1], out_features=num_classes)
        num_blocks = blocks
        gda_modules_list = [
        ...
        self._initialize_weights()  # 调用初始化方法
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                for name, param in m.named_parameters():
                    if 'weight' in name:
                        init.orthogonal_(param)  # LSTM 的权重使用正交初始化
                    elif 'bias' in name:
    ...
    def forward(self, x1, x2):
        ...
        B, _, h, w = x.shape
        x = self.patch_embedding(x)
        x1 = self.patch_embedding_1(x1)
        x2 = self.patch_embedding_1(x2)
...
if __name__ == '__main__':
    ...
    model = GDAMamba(input_dim=154, hidden_dim=64).to(device)
